...
if __name__ == "__main__":
    ...
    def get_profile_lines(patient_profiles, drug_name, summary, risk_score):
        ...
        global_valid_format = True
        for i in range(0, len(matches)):
            profile = matches[i][2:]
            valid_format = True
            print(not "Patient factual information" in profile or len(profile) < 30)
            ...
            print(not "Reason why the drug is indicated or contraindicated" in justification or len(justification) < 53)
            ...
            if valid_format:
                ...
                print(f"Invalid format for profile {i}: {profile}")
            global_valid_format = bool(len(list_profile) > 0)
        return list_profile, global_valid_format
    ...
    def generate_profiles(client, drug_name, tokenizer):
        ...
        if not valid_format_cat1 or not valid_format_cat1 or not valid_format_cat2:
    ...
    concatenated_df = pd.read_csv(
        "/home/tom/Bureau/phd/mistral_training/hackaton_medi/generation_mock_dataset/genearated_dataset/concatenated_dataset.csv")
    ...
    for i in tqdm(list(range(0, 50))):
        ...
        if valid_format:
            ...
            print("valid format length", len(list_valide_format))
        else:
    ...
    df.to_csv(f'/home/tom/Bureau/phd/mistral_training'
              f'/hackaton_medi/dataset_finetune/profile_{random_int}.csv', index=False)
